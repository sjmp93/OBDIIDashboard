...
public class SettingsActivity extends PreferenceActivity implements Preference.OnPreferenceChangeListener {
    ...
    public static int getObdUpdatePeriod(SharedPreferences prefs) {
        ...
        try {
            period = (int) (Double.parseDouble(periodString)); // El valor informado en la configuración es el que usaremos
        } catch (Exception e) {
        }
    ...
    public static double getVolumetricEfficieny(SharedPreferences prefs) {
        String veString = prefs.getString(SettingsActivity.VOLUMETRIC_EFFICIENCY_KEY, ".85");
        double ve = 0.85;
        try {
    ...
    public static double getEngineDisplacement(SharedPreferences prefs) {
        String edString = prefs.getString(SettingsActivity.ENGINE_DISPLACEMENT_KEY, "1.6");
        double ed = 1.6;
        try {
    ...
    public static double getMaxFuelEconomy(SharedPreferences prefs) {
        String maxStr = prefs.getString(SettingsActivity.MAX_FUEL_ECON_KEY, "70");
        double max = 70;
        try {
    ...
    protected static int getMaxRPM(SharedPreferences prefs){
        ...
        int rpm = 7500; // by default 7500rpm

        try {
            rpm = (int) (Double.parseDouble(rpmString)); // El valor informado en la configuración es el que se usa
        } catch (Exception e) {
        }
